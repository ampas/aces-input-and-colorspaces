...
// <ACEStransformID>urn:ampas:aces:transformId:v2.0:CSC.Academy.ADX16_to_ACES.a2.v1</ACEStransformID>
// <ACESuserName>ADX16 to ACES2065-1</ACESuserName>

//
// ACES Color Space Conversion - ADX16 to ACES2065-1
//
// converts ADX16 (Acdemy Denisty Exchange Encoding 16-bits) to
    ...
    output varying float aOut)
{
    ...
    float logE[3];
    if (cid[0] <= 0.6)
        logE[0] = interpolate1D(LUT_1D, cid[0]);
    if (cid[1] <= 0.6)
        logE[1] = interpolate1D(LUT_1D, cid[1]);
    if (cid[2] <= 0.6)
        logE[2] = interpolate1D(LUT_1D, cid[2]);

    if (cid[0] > 0.6)
        logE[0] = (100.0 / 55.0) * cid[0] - REF_PT;
    if (cid[1] > 0.6)
        logE[1] = (100.0 / 55.0) * cid[1] - REF_PT;
    if (cid[2] > 0.6)
        logE[2] = (100.0 / 55.0) * cid[2] - REF_PT;
